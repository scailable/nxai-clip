...
def run_pytorch_inference():
    ...
    text_tokens = torch.tensor(tokenizer.tokenize_batch(TEXT_CLASSES, context_length=77), dtype=torch.int32).to(device)
    ...
    torch.onnx.export(
        ...
        dynamic_axes={
            ...
            "probabilities": {1: "num_prompts"}  # The output probabilities second dim corresponds to text prompts
        },
        opset_version=14,
...
def infer_directory(ort_session, images_dir, text_tokens_np):
    files = [f for ext in ("*.png", "*.jpg", "*.jpeg") for f in glob(os.path.join(images_dir, ext))]
    if not files:
